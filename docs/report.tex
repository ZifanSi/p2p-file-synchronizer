...
\section{How to Run}
\subsection*{Optional reset scripts}
...
\textbf{Steps:}
\begin{enumerate}[noitemsep]
  \item Run \texttt{src\textbackslash{}bats\textbackslash{}clean\_peer.bat} to reset state.
...
\textbf{Steps:}
\begin{enumerate}[noitemsep]
  \item Stop all peers (or run \texttt{src\textbackslash{}bats\textbackslash{}clean\_port.bat}).
  \item Append one line to \texttt{src/Peer2/fileB.txt} and save (mtime increases).
  \item Restart tracker and peers.
  \item Record mtimes using the provided helper script:
\begin{lstlisting}
cd src
...
\textbf{Observed:} After restart and synchronization, Peer1 and Peer3 matched Peer2's updated file (same hash via \texttt{compare\_files.py}) and mtimes (via \texttt{tc\_4.py}).
...
\textbf{Steps:}
\begin{enumerate}[noitemsep]
  \item Start peers and identify the serving peer port for \texttt{fileC.txt} (Peer3 in this run used port 8002).
  \item Run:
\begin{lstlisting}
cd src
