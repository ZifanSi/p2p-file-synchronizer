...
\title{Assignment 2: Peer-to-Peer File Synchronizer\\Report (Base + Patch Feb 27)}
...
\noindent The complete repository, including source code, documentation, and test files, is available on GitHub: \\
{\Large \color{blue} \url{https://github.com/ZifanSi/p2p-file-synchronizer}}

\vspace{0.5em}
\noindent Source files and scripts are located in the \path{src/} directory, documentation are located in the \path{docs/} directory.


\section{Overview}
This P2P file synchronizer uses a central tracker to coordinate file sharing and keep peer directories in sync.

Peer workflow:
\begin{itemize}[noitemsep]
\item Connects to the tracker via a persistent TCP connection.
\item Sends local file names and timestamps to the tracker.
\item Sends keepalive signals every 5 seconds to stay active.
\item Downloads missing or outdated files discovered through the tracker.
\item Acts as a server to send files to other peers using a size header.
\end{itemize}
...
\noindent \textbf{File Structure Before:}
\begin{itemize}
    \item \path{src/}
    \begin{itemize}
        \item \path{Peer1/} (fileA.txt)
        \item \path{Peer2/} (fileB.txt)
        \item \path{Peer3/} (fileC.txt)
    \end{itemize}
\end{itemize}


\noindent \textbf{File Structure After:}
\begin{itemize}
    \item \path{src/}
    \begin{itemize}
        \item \path{Peer1/} (fileA.txt, fileB.txt, fileC.txt)
        \item \path{Peer2/} (fileA.txt, fileB.txt, fileC.txt)
        \item \path{Peer3/} (fileA.txt, fileB.txt, fileC.txt)
    \end{itemize}
\end{itemize}
...
\textbf{Observed:} After restart and synchronization, \texttt{fileB.txt} on all peers had the same integer modified time and size, indicating that the newer version propagated and that the receiving peers preserved the advertised mtime via \texttt{os.utime()}.
\begin{lstlisting}
> python tests\tc_4.py Peer2\fileB.txt
Peer2\fileB.txt: mtime=1771289244 size=38

> python tests\tc_4.py Peer1\fileB.txt
Peer1\fileB.txt: mtime=1771289244 size=38

> python tests\tc_4.py Peer3\fileB.txt
Peer3\fileB.txt: mtime=1771289244 size=38
\end{lstlisting}

\textbf{Result:} PASS.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{tc4.png}
    \caption{tc4}
    \label{fig:tc4}
\end{figure}
...
\textbf{Observed:} After restarting Peer3 and allowing synchronization to complete, \texttt{big.bin} existed on all three peers and had the same size.
\begin{lstlisting}
"C:\Users\sifra\Desktop\p2p\src\Peer1\big.bin" 10240kb
"C:\Users\sifra\Desktop\p2p\src\Peer2\big.bin" 10240kb
"C:\Users\sifra\Desktop\p2p\src\Peer3\big.bin" 10240kb
\end{lstlisting}

\textbf{Result:} PASS.

\section{Patch Update (Feb 27)}
I made a few small fixes to improve error handelling in \path{src/fileSynchronizer.py}. Some were around the template. This section briefly explains what changed and how I checked it.

\subsection{PF1: Better free-port check}
\textbf{Issue:} My earlier version checked port availability by searching the output of \texttt{netstat}. That approach is not very reliable, because a port number can appear as part of a different number (for example, \texttt{8000} inside \texttt{18000}).

\textbf{Fix:} I changed \texttt{check\_port\_avaliable()} so it creates a temporary socket and tries to bind directly to \texttt{("0.0.0.0", port)}. If the bind works, the port is free.

\textbf{Quick check:}
\begin{enumerate}[noitemsep]
  \item Start Peer1, Peer2, and Peer3 from a clean state.
  \item Check that each peer gets its own listening port.
\end{enumerate}
...
\subsection{PF2: One failed download should not stop the whole sync}
\textbf{Issue:} In the earlier version, if one file download failed, it could interrupt the current sync cycle.

\textbf{Fix:} I updated the \texttt{sync()} logic so that a failed \texttt{syncfile(...)} call is skipped, and the peer continues checking the rest of the files instead of stopping.

\textbf{Quick check:}
\begin{enumerate}[noitemsep]
  \item Use the TC6 setup with \texttt{big.bin} stored on Peer3.
  \item Interrupt Peer3 while Peer1 is downloading the file.
  \item Leave the other peers running and watch the next sync cycles.
\end{enumerate}

\textbf{Observed:} A failed transfer no longer stops later sync attempts. After Peer3 was started again, the missing file was downloaded successfully in a later cycle.

\textbf{Result:} PASS.

\subsection{PF3: Safer server accept loop}
\textbf{Issue:} The peer server loop originally accepted incoming connections without much protection. If something went wrong during \texttt{accept()} or thread startup, the peer could exit unexpectedly.

\textbf{Fix:} I added basic \texttt{try/except} handling around \texttt{accept()} and around the thread start, with a safe close if a connection object had already been created.

\textbf{Quick check:}
\begin{enumerate}[noitemsep]
  \item Run the tracker and all peers normally.
  \item Let them exchange files for several sync cycles.
  \item Run TC5 at the same time to create extra peer requests.
\end{enumerate}

\textbf{Observed:} The peers continued serving requests normally and stayed running during repeated sync cycles.

\textbf{Result:} PASS.

\subsection{Patch sanity check}
To make sure the updated file still parsed correctly after the changes, I also ran a quick syntax check.

\textbf{Command run:}
\begin{lstlisting}
python -c "import ast, pathlib; ast.parse(pathlib.Path('src/fileSynchronizer.py').read_text(encoding='utf-8')); print('OK')"
\end{lstlisting}
...
PF1: Patch check - reliable free-port detection & PASS \\
PF2: Patch check - skip failed download, continue sync & PASS \\
PF3: Patch check - guarded accept/thread handling & PASS \\
\bottomrule
\end{tabular}
\end{center}
