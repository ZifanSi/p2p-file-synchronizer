...
\section{How to Run}
\subsection*{Reset scripts (recommended for repeatable tests)}
\begin{lstlisting}
cd C:\Users\sifra\Desktop\p2p-file-synchronizer
src\bats\clean_peer.bat (will reset peer folders to initial state with only fileA/B/C)
src\bats\clean_port.bat (will stop all peers and tracker, useful for testing port binding and clean restarts)
\end{lstlisting}

\subsection*{Start tracker + peers (opens multiple terminals)}
\begin{lstlisting}
cd C:\Users\sifra\Desktop\p2p-file-synchronizer
src\run_all.bat
\end{lstlisting}
